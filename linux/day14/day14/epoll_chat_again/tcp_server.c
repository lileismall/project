#include <func.h>
int tcpInit(int *sFd,char* ip,char* port)//初始化
{
    int socketFd;
    ...
    int ret;
    int reuse=1;
    ret=setsockopt(socketFd,SOL_SOCKET,SO_REUSEADDR,&reuse,sizeof(int));//设置套接口socketFd的属性，注意要放在bind函数前
    ERROR_CHECK(ret,-1,"setsockopt");
    //将指定ip和port和套接口绑定，或者说是相关联
    ...
    *sFd=socketFd;//sFD是传出参数,也可以用返回的形式将套接口返回
    return 0;
}
    ...
    while(1)
    {
        ...
        for(i=0;i<readyFdCount;i++)
        {
            if(socketFd==evs[i].data.fd)
            {
                ...
                newFd=accept(socketFd,(struct sockaddr*)&clientAddr,&addrLen);//将客户端的信息接住
